...
pub fn show_sym<F>(f: &mut F, state: &mut OutputState, i: &hir::SymRef)
where
    F: Write,
{
...
pub fn show_type_kind<F>(f: &mut F, state: &mut OutputState, t: &hir::TypeKind)
where
    F: Write,
{
    match *t {
...
pub fn show_type_specifier<F>(f: &mut F, state: &mut OutputState, t: &syntax::TypeSpecifier)
where
    F: Write,
{
...
pub fn show_type<F>(f: &mut F, state: &mut OutputState, t: &Type)
where
    F: Write,
{
...
pub fn show_struct<F>(f: &mut F, state: &mut OutputState, s: &syntax::StructSpecifier)
where
    F: Write,
{
...
pub fn show_array_sizes<F>(f: &mut F, a: &hir::ArraySizes)
where
    F: Write,
{
    panic!()
...
pub fn show_arrayed_identifier<F>(f: &mut F, ident: &syntax::Identifier, ty: &hir::Type)
where
    F: Write,
{
    let _ = write!(f, "{}", ident);
...
pub fn emit_type(state: &mut OutputState, ty: &hir::Type) -> Word {
    ...
    match ty.kind {
        hir::TypeKind::Float => emit_float(state),
        hir::TypeKind::Double => {
            //XXX: actually use double here
            emit_float(state)
        }
        hir::TypeKind::Vec4 => emit_vec4(state),
        _ => panic!("{:?}", ty.kind),
    }
}
...
pub fn translate_lvalue_expr(state: &mut OutputState, expr: &hir::Expr) -> Word {
    match expr.kind {
        hir::ExprKind::Variable(s) => emit_sym(state, s),
        _ => panic!(),
    }
}
...
pub fn translate_vec4(
    ...
    state
        .builder
        .composite_construct(float_vec4, None, args)
        .unwrap()
}

pub fn translate_r_val(state: &mut OutputState, expr: &hir::Expr) -> Word {
    match expr.kind {
        hir::ExprKind::FunCall(ref fun, ref args) => match fun {
        ...
        hir::ExprKind::Variable(sym) => {
            ...
            state
                .builder
                .load(v.ty, None, v.location, None, [])
                .unwrap()
        }
...
pub fn show_hir_expr<F>(f: &mut F, state: &mut OutputState, expr: &hir::Expr)
where
    F: Write,
{
    match expr.kind {
...
pub fn show_hir_function_identifier<F>(f: &mut F, state: &mut OutputState, i: &hir::FunIdentifier)
where
    F: Write,
{
    match *i {
...
pub fn show_declaration<F>(f: &mut F, state: &mut OutputState, d: &hir::Declaration)
where
    F: Write,
{
    match *d {
...
pub fn show_function_prototype<F>(f: &mut F, state: &mut OutputState, fp: &hir::FunctionPrototype)
where
    F: Write,
{
...
pub fn show_function_parameter_declarator<F>(
    f: &mut F,
    state: &mut OutputState,
    p: &hir::FunctionParameterDeclarator,
) where
    F: Write,
{
...
pub fn show_init_declarator_list<F>(f: &mut F, state: &mut OutputState, i: &hir::InitDeclaratorList)
where
    F: Write,
{
...
pub fn translate_single_declaration(state: &mut OutputState, d: &hir::SingleDeclaration) {
    ...
    let storage = match &state.hir.sym(d.name.unwrap()).decl {
        hir::SymDecl::Variable(storage, _) => match storage {
...
pub fn show_single_declaration<F>(f: &mut F, state: &mut OutputState, d: &hir::SingleDeclaration)
where
    F: Write,
{
...
pub fn show_initializer<F>(f: &mut F, state: &mut OutputState, i: &hir::Initializer)
where
    F: Write,
{
    match *i {
...
pub fn show_block<F>(f: &mut F, state: &mut OutputState, b: &hir::Block)
where
    F: Write,
{
...
pub fn show_compound_statement<F>(f: &mut F, state: &mut OutputState, cst: &hir::CompoundStatement)
where
    F: Write,
{
    let _ = f.write_str("{\n");
...
pub fn show_statement<F>(f: &mut F, state: &mut OutputState, st: &hir::Statement)
where
    F: Write,
{
    match *st {
...
pub fn show_simple_statement<F>(f: &mut F, state: &mut OutputState, sst: &hir::SimpleStatement)
where
    F: Write,
{
    match *sst {
...
pub fn show_expression_statement<F>(f: &mut F, state: &mut OutputState, est: &hir::ExprStatement)
where
    F: Write,
{
...
pub fn show_selection_rest_statement<F>(
    f: &mut F,
    state: &mut OutputState,
    sst: &hir::SelectionRestStatement,
) where
    F: Write,
{
    match *sst {
...
pub fn show_switch_statement<F>(f: &mut F, state: &mut OutputState, sst: &hir::SwitchStatement)
where
    F: Write,
{
...
pub fn show_case_label<F>(f: &mut F, state: &mut OutputState, cl: &hir::CaseLabel)
where
    F: Write,
{
    match *cl {
...
pub fn show_condition<F>(f: &mut F, state: &mut OutputState, c: &hir::Condition)
where
    F: Write,
{
    match *c {
...
pub fn show_for_init_statement<F>(f: &mut F, state: &mut OutputState, i: &hir::ForInitStatement)
where
    F: Write,
{
    match *i {
...
pub fn show_for_rest_statement<F>(f: &mut F, state: &mut OutputState, r: &hir::ForRestStatement)
where
    F: Write,
{
...
pub fn show_jump_statement<F>(f: &mut F, state: &mut OutputState, j: &hir::JumpStatement)
where
    F: Write,
{
    match *j {
...
pub fn translate_external_declaration(state: &mut OutputState, ed: &hir::ExternalDeclaration) {
    match *ed {
        ...
        hir::ExternalDeclaration::FunctionDefinition(ref fd) => {
            translate_function_definition(state, fd)
        }
